...
namespace stoat {
    ...
    void CorrectionHistoryTable::update(const Position& pos, i32 depth, Score searchScore, Score staticEval) {
        ...
        m_kgrTable[pos.stm().idx()][pos.kgrKey() % kEntries].update(bonus);
    }
    ...
    i32 CorrectionHistoryTable::correction(const Position& pos) const {
        ...
        correction += m_ksbTable[pos.stm().idx()][pos.ksbKey() % kEntries];
        correction += m_kgrTable[pos.stm().idx()][pos.kgrKey() % kEntries];
